class Spree::Address::FormPresenter < Spree::CommonPresenter
    ...
    def state_elements
        select = "<select disabled='#{!have_states ? 'disabled' : ''}' name='address[state_id]' class='#{have_states ? 'form-control required' : 'form-control hidden'}'><option value=''></option>"
        @address.country.states.each do |s|
            select << "<option value='#{s.id}'>#{s.name}</option>"
        end
        select << "</select>"
        input = "<input disabled='#{have_states ? 'disabled' : ''}' type='text' name='address[state_name]' class='#{!have_states ? 'form-control required' : 'form-control hidden'}'>"
        [select + input].join.gsub('"', "'").gsub("\n", "")
    end
